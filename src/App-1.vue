        ...
        <div class="relative">
          ...
          < <div ref="carousel"
            ...
            <div v-for="(item, index) in cards" :key="index"
              class="flex-shrink-0 w-[85%] sm:w-[320px] md:w-[380px] lg:w-[420px] snap-center bg-white border border-gray-200 rounded-lg shadow flex items-center gap-3 sm:gap-4 p-3 sm:p-4 h-[110px] sm:h-[130px] md:h-[120px] lg:h-[100px] transition-transform duration-300">
              <img :src="item.img" :alt="item.title"
                class="w-[70px] h-[70px] sm:w-[90px] sm:h-[90px] md:w-[100px] md:h-[100px] object-contain" />
  ...
  <section class="py-16 px-4 bg-gray-50">
    <div class="max-w-7xl mx-auto px-6">
      ...
      <div v-for="(item, index) in faqList" :key="index"
        class="mb-4 rounded-xl shadow-md overflow-hidden transition-shadow duration-300 hover:shadow-xl">
        <button @click="toggle(index)"
          class="w-full px-6 py-5 flex justify-between items-center text-left text-lg font-medium text-gray-800 duration-200">
        ...
        <transition name="fade-slide">
          <div v-if="activeIndex === index" class="px-6 pb-5 text-gray-600 text-sm italic">
            {{ item.answer }}
          </div>
        </transition>
      </div>
